...
def wolff_update_2d_ising(config, beta, N):
    ...
    Fixed implementation that correctly flips the seed spin and grows the cluster.
    """
    ...
    # Flip the seed spin (THIS WAS MISSING IN ORIGINAL)
    config[i, j] = -s0
...
def measure2d_all(config, N, model):
    ...
    #             for r in range(R+1):
    #                 corr[r] += math.cos(theta - config[(i+r)%N, j])
...
def measure3d_all(config, N, model):
    ...
    #                 sum_sin += math.sin(theta)
    #                 for r in range(R+1):
    #                     corr[r] += math.cos(theta - config[(x+r)%N, y, z])
    #     mag = math.sqrt(sum_cos * sum_cos + sum_sin * sum_sin)
    #     norm = N * N * N
    #     for r in range(R+1):
    #         corr[r] /= norm
    ...
    #                 s1 = config[x, y, z, 1]
    #                 s2 = config[x, y, z, 2]
    #                 energy += - ((s0 * config[(x+1)%N, y, z, 0] +
    #                               s1 * config[(x+1)%N, y, z, 1] +
    #                               s2 * config[(x+1)%N, y, z, 2]) +
    #                              (s0 * config[x, (y+1)%N, z, 0] +
    #                               s1 * config[x, (y+1)%N, z, 1] +
    #                               s2 * config[x, (y+1)%N, z, 2]) +
    #                              (s0 * config[x, y, (z+1)%N, 0] +
    #                               s1 * config[x, y, (z+1)%N, 1] +
    #                               s2 * config[x, y, (z+1)%N, 2]))
    #                 sum0 += s0
    #                 sum1 += s1
    #                 sum2 += s2
    #                 for r in range(R+1):
    #                     corr[r] += (s0 * config[(x+r)%N, y, z, 0] +
    #                                 s1 * config[(x+r)%N, y, z, 1] +
    #                                 s2 * config[(x+r)%N, y, z, 2])
    #     mag = math.sqrt(sum0 * sum0 + sum1 * sum1 + sum2 * sum2)
    #     norm = N * N * N
    #     for r in range(R+1):
    #         corr[r] /= norm
...
if __name__ == '__main__':
    output_folder = "data23_test"
    nt          = 30                     # Number of temperature points
    n_list      = [16, 32]       # Lattice sizes
    eqSteps     = 1024 * 100              # Equilibration sweeps
    mcSteps     = 1024 * 10               # Measurement sweeps
    T_arr       = np.linspace(0, 4, nt)   # Temperature array
    dim         = 2                       # 2 or 3
